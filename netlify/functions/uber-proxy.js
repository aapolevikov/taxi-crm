...
// v9.3: performance-data с перебором форматов body (?fmt=A|B|C|D|E|F)
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // === 📊 v9.3: PERFORMANCE DATA через analytics-data/query ===
    ...
    // Uber ругается что orgId должен быть объектом, не строкой — пробуем варианты через ?fmt=
    if (action === 'performance-data') {
      ...
      // Различные форматы body для перебора
      let body;
      if (fmt === 'A') {
        // orgId как объект {id}
        body = {
        ...
        // orgId как объект {value}
        body = {
          request: {
            orgId: { value: apiOrgId },
            dimension: dimension,
            startTime: String(startTime),
            endTime: String(endTime),
            pagination: { pageSize: 50 }
        ...
        // org_id snake_case как объект {id}
        body = {
          request: {
            org_id: { id: apiOrgId },
            dimension: dimension,
        ...
        // Без обёртки request, orgId как объект
        body = {
          orgId: { id: apiOrgId },
          dimension: dimension,
          startTime: String(startTime),
        ...
        // request с encryptedId
        body = {
          request: {
            orgId: { encryptedId: apiOrgId },
            dimension: dimension,
            startTime: String(startTime),
            endTime: String(endTime),
            pagination: { pageSize: 50 }
        ...
        // request с orgId.encrypted_id
        body = {
          request: {
            orgId: { encrypted_id: apiOrgId },
            dimension: dimension,
            startTime: String(startTime),
            endTime: String(endTime),
            pagination: { pageSize: 50 }
        ...
        return { statusCode: 400, headers, body: JSON.stringify({ error: 'unknown fmt, try A/B/C/D/E/F' }, null, 2) };
      }
      ...
      // Возвращаем с указанием формата для отладки
      return { 
    ...
    return { 
      ...
      headers, 
      body: JSON.stringify({
        message: 'Uber API proxy v9.3 - performance-data with body format probe',
        actions: {
