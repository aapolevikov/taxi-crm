...
// v9.4: performance-data — формат с reportRequests[]
...
// performance: /v1/vehicle-suppliers/analytics-data/query (POST с reportRequests[])
  ...
  try {
    ...
    if (action === 'performance-data') {
      ...
      if (fmt === 'A') {
        ...
        return { statusCode: 400, headers, body: JSON.stringify({ error: 'unknown fmt, try A/B/C/D/E' }, null, 2) };
      }
    ...
    return { 
      ...
      headers, 
      body: JSON.stringify({
        message: 'Uber API proxy v9.4 - performance-data with reportRequests[]',
        actions: {
