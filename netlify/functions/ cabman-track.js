...
async function appendHistory(vehicles, event){
  ...
  // ── Метка времени = момент реального опроса Cabman (НЕ v.time/gmt прибора,
  //    который у стоящих машин «застывает» на старой дате). Так ключ-дата
  //    всегда совпадает с тем, что ищет action=history (день по UTC). ──
  const nowIso = new Date().toISOString();
  const today  = dayKey(nowIso);
  // ключ дедупа — округление до минуты: один запуск раз в минуту = одна точка,
  // даже если машина стоит с одинаковыми координатами (нужно для подсчёта стоянок)
  const nowMinute = nowIso.slice(0,16); // 'YYYY-MM-DDTHH:MM'
  for(const v of vehicles){
    ...
    // de-dup: пропускаем, только если за эту же минуту точка уже записана
